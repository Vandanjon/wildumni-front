    ...
    <div id="AdminPage" className="pageContainer">
      ...
      <main>
        {users ? (
          <>
            <Link to="/register">
              <Fab size="small" color="primary" aria-label="addUser">
                <AddIcon />
              </Fab>
            </Link>
            ...
            >
              {users.map((user) => (
                <ListItem className="userLine" button key={user.id}>
      ...
      <footer>
        {connectedUser ? `Welcome ${connectedUser.firstName}` : "Welcome Admin"}
      </footer>
    </div>
  );
};
